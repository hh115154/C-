...
{
	/*给结点附加一个计数属性time，初始化 t.time = 1，
	其它的 time 均为 0 ，以 s 为源结点进行 DFS，
	一旦搜索到 t，则 t 马上着黑色（即不继续搜索 t 的后代）。
	每当结束一个结点的搜索，则该结点的 time 属性 = 该结点所指向的所有结点的 time 之和。
	最后 s.time 即路径条数。*/
	for (int i = 0; i < nodeNum; i++)
	{
		...
		while (nb) {
			...
			if (Adj[nb->num-1]->dist>minNode->dist+nb->eLen)
			{
				...
				if (Color::WHITE ==Adj[nb->num-1]->color)
				{
					...
					Adj[nb->num - 1]->color = Color::GRAY;//使用颜色限制，保证每个节点只入堆一次
				}		
			}
			//nb.relaxation end
			nb = nb->neighbor;
...
/*
* leetcode542
* 算法描述：01矩阵中每个节点与上下左右相连且距离1，求出每个1节点离最近0节点的最短距离
* 1.init each 0 as color of gray and push into queue
* 2.BFS the metrix graph,each layer distance++
*/

	metGraph.resize(l, vector<MetrixNode*>(c, nullptr));//vector 初始化
